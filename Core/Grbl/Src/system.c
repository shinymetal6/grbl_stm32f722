...
#else
    if (bit_isfalse(pin,CONTROL_RESET_BIT)) { control_state |= CONTROL_PIN_INDEX_RESET; }
    if (bit_isfalse(pin,CONTROL_FEED_HOLD_BIT)) { control_state |= CONTROL_PIN_INDEX_FEED_HOLD; }
    if (bit_isfalse(pin,CONTROL_CYCLE_START_BIT)) { control_state |= CONTROL_PIN_INDEX_CYCLE_START; }
#endif
  }
