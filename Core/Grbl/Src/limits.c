...
#endif
{
	...
	if (HAL_NVIC_GetPendingIRQ(Y_LIMIT_BIT) != RESET)
	{
		HAL_NVIC_ClearPendingIRQ(Y_LIMIT_BIT);
	}
	if (HAL_NVIC_GetPendingIRQ(Z_LIMIT_BIT) != RESET)
	{
		HAL_NVIC_ClearPendingIRQ(Z_LIMIT_BIT);
	}
